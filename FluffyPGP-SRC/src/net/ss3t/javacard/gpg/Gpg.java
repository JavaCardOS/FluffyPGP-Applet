...
public final class Gpg extends Applet {
  ...
  public void process(APDU apdu) {
    ...
    if (ins == CMD_GET_RESPONSE) {//0xC0
    	//if commandChainingBuffer[0] == 0x47
      if (commandChainingBuffer[TEMP_INS] == CMD_GENERATE_ASYMETRIC) {
        short lengthLeftToSend = Util.getShort(commandChainingBuffer, TEMP_GET_RESPONSE_LENGTH);//3
        ...
        short offset = Util.getShort(commandChainingBuffer, TEMP_GET_RESPONSE_OFFSET);//1
    ...
    if (commandChainingBuffer[TEMP_INS] != ins) {//0
	...
	//Select Applet,return the length of JCRE AID
    if (selectingApplet()) {
    ...
    switch (ins) {
      case CMD_VERIFY://0x20
        verify(apdu);
        break;

      case CMD_CHANGE_REFERENCE_DATA://0x24
        changeReferenceData(apdu);
        break;

      case CMD_RESET_RETRY_COUNTER://0x2C
        resetRetryCounter(apdu);
        break;

      case CMD_GET_DATA://0xCA
        getData(apdu);
        break;

      case CMD_COMPUTE_PSO://0x2A
      ...
      case CMD_GENERATE_ASYMETRIC://0x47
      ...
      case CMD_PUT_DATA://0xDA
        putData(apdu);
        break;

      case CMD_GET_CHALLENGE://0x84
        getChallenge(apdu);
        break;

      case CMD_PUT_KEY://0xDB
      ...
      case CMD_INTERNAL_AUTHENTICATE://0x88
        if (p1p2 != (short) 0) {
          ISOException.throwIt(ISO7816.SW_INCORRECT_P1P2);
        }
      ...
      case CMD_TERMINATE_DF://0xE6
        terminateDF(apdu);
        break;

      case CMD_ACTIVATE_FILE://0x44
        activateFile(apdu);
        break;
   ...
   * CHANGE REFERENCE DATA APDU implementation.
   P1=0, P2=83(PIN_INDEX_PW3/admin, 1)/81(PIN_INDEX_PW1/user,0)
   */
  ...
  private short addKeyPart(byte[] data, short offset) {
  	//commandChainingBuffer[4], commandChainingBuffer[3]
  ...
  private void putKey(APDU apdu) {
    ...
    //第一条PUT KEY APDU时，将Data域中的数据拷贝至commandChainingBuffer
    if (firstCommand) {
      ...
      Util.arrayFillNonAtomic(commandChainingBuffer, (short) 0,
                              (short) commandChainingBuffer.length, (byte) 0);
      //commandChainingBuffer[3]=buffer[9]
      commandChainingBuffer[TEMP_PUT_KEY_KEY_TYPE] = buffer[(short) (ISO7816.OFFSET_CDATA + 4)];
      commandChainingBuffer[TEMP_PUT_KEY_EXPECTED_CHUNK_SIZE] = 0;//commandChainingBuffer[5]=0
      ...
      JCSystem.beginTransaction();
      KeyPair key = getKey(commandChainingBuffer[TEMP_PUT_KEY_KEY_TYPE]);//commandChainingBuffer[3]
    ...
    //非第一条PUT KEY命令
    else //commandChainingBuffer[TEMP_INS] == buffer[ISO7816.OFFSET_INS] 
    {
      // Chained command.
      pos = ISO7816.OFFSET_CDATA;//5
    }
	//acc = commandChainingBuffer[1]
    short accumulatorLength = Util.getShort(commandChainingBuffer, TEMP_PUT_KEY_ACCUMULATOR_LENGTH);
    //for (; commandChainingBuffer[4] <= 6; ) {
    for (; commandChainingBuffer[TEMP_PUT_KEY_KEY_CHUNK] <= KEY_PART_N; ) {
      ...
      if (accumulatorLength != 0) {	
        ...
        if (left < sizeNeeded) {
          ...
          // We need an extra APDU with more data. commandChainingBuffer[0]=0xDB
          commandChainingBuffer[TEMP_INS] = CMD_PUT_KEY;
          return;
        } else {
  ...
  private void generateAsymetricKey(APDU apdu) {
    ...
    if (commandChainingBuffer[TEMP_GET_RESPONSE_DATA] == 0) {//若==0 则GetModulus失败
      Util.setShort(commandChainingBuffer, TEMP_GET_RESPONSE_OFFSET,
